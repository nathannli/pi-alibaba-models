...
// All capabilities are id-inferred; users can correct the context window via
// `contextWindowOverrides` in alibaba-config.json.
function inferPlanDef(
	...
	// maxTokens: openaiOnly tier (16384) > default (65536).
...
async function fetchCloudModels(
	...
	const models = rows
		...
		.map((m) => {
			...
			const maxTokens = m.maxTokens && m.maxTokens > 0 ? m.maxTokens : 8192;
			return {
				id: m.id,
				name: m.name || m.id,
				reasoning: isReasoning,
